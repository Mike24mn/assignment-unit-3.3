...
// ***** STRETCH GOALSsupplyChanges.length; index ++;)write the 'for' loop from #6 as a 'for...of' loop. Instead of 'positives',
//    'negatives', and 'zeroes', create three new arrays named 'stretchPositives',
//    'stretchNegatives', and 'stretchZeroe{
...
for(let numIs of supplyChanges) {
  console.log('for of loop', numIs);
  if(numIs > 0) {
    stretchPositives.push(numIs);
  }
  else if (numIs < 0) {
    stretchNegatives.push(numIs);
  }
  else {stretchZeroe.push(numIs);
  }
}

console.log(stretchPositives);
console.log(stretchNegatives);
console.log(stretchZeroe);
...
while (parts > 6) {
  parts -= 7; // subtract 7 from parts
  boxesFilled ++; // increment boxes filled by 1
}
